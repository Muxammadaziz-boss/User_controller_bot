...
def install_package(pip_name: str, silent: bool = True) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(f"✅ O'rnatildi: {pip_name}")
            return True
        else:
            logger.warning(f"⚠️ O'rnatilmadi: {pip_name}")
            return False
            
    except subprocess.TimeoutExpired:
        logger.error(f"⏱️ Timeout: {pip_name}")
        return False
    except Exception as e:
        logger.error(f"❌ O'rnatish xatosi ({pip_name}): {e}")
        return False
...
def install_missing_packages(packages: List[Tuple[str, str, bool]], silent: bool = True) -> dict:
    ...
    for import_name, pip_name, is_critical in packages:
        logger.info(f"📦 O'rnatilmoqda: {pip_name}...")
        ...
        if is_critical and not success:
            logger.error(f"❌ CRITICAL: {pip_name} o'rnatilmadi!")
...
def _install_in_background(packages: List[Tuple[str, str, bool]]):
    ...
    try:
        logger.info(f"🔄 Background: {len(packages)} ta kutubxona o'rnatilmoqda...")
        results = install_missing_packages(packages, silent=True)
        
        success_count = sum(1 for v in results.values() if v)
        fail_count = len(results) - success_count
        
        logger.info(f"✅ Background o'rnatish tugadi: {success_count} OK, {fail_count} FAIL")
        
    except Exception as e:
        logger.error(f"❌ Background xatosi: {e}")
...
def ensure_dependencies(blocking: bool = False) -> bool:
    ...
    try:
        ...
        if not missing:
            logger.debug("✅ Barcha kutubxonalar mavjud")
            return True
        ...
        logger.info(f"📦 Yetishmayotgan: {len(critical_missing)} critical, {len(optional_missing)} optional")
        
        # 3. Critical kutubxonalarni darhol o'rnatish (blocking)
        if critical_missing:
            logger.info("⚠️ Critical kutubxonalar o'rnatilmoqda...")
            results = install_missing_packages(critical_missing, silent=True)
            
            # Tekshirish
            for import_name, pip_name, _ in critical_missing:
                if not check_package(import_name):
                    logger.error(f"❌ CRITICAL yetishmaydi: {import_name}")
                    return False
        ...
        if optional_missing:
            if blocking:
                ...
                thread.start()
                logger.info("🔄 Optional kutubxonalar background'da o'rnatilmoqda...")
        
        return True
        
    except Exception as e:
        logger.error(f"❌ ensure_dependencies xatosi: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        missing = get_missing_packages()
        if missing:
            print(f"\n❌ Yetishmayotgan: {len(missing)} ta")
            for i, p, c in missing:
                print(f"  • {p}")
        else:
            print("\n✅ Barcha kutubxonalar mavjud!")
            
    elif args.install:
        print("\n📦 O'rnatish boshlanmoqda...")
