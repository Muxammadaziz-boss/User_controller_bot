# bot.py - v3.0.2 SERVER BOT (PART 1/2)
"""
Central server bot:
- Device management
...
from ai_helper import (
    initialize_ai, ai_chat, ai_explain_error, 
...
async def message_handler(event):
    ...
    try:
        ...
        if is_ai_enabled() and message:
            # Parse command
            from ai_helper import ai_parse_command, extract_device_number
            
            parsed = ai_parse_command(message)
            
            if parsed.get('is_command'):
                # AI command aniqladi
                await handle_ai_command(event, parsed)
            else:
                # Oddiy suhbat
                context = {
                    'devices': device_registry.get_statistics(),
                    'timestamp': datetime.now().isoformat()
                }
                
                response = await ai_chat(message, context)
...
async def show_ai_settings(event):
    ...
    try:
        ...
        ai_type = ai_status.get('type', 'Unknown')
        if ai_type == 'OpenRouterAI':
            ai_model = "OpenRouter (Gemini 2.0 Free)"
        elif ai_type == 'GeminiAI':
        ...
        message = (
            ...
            f"Model: {ai_model}\n"
            f"Conversations: {ai_status.get('conversation_length', 0)}\n\n"
            f"**Features:**\n"
